"""dN/dS data loading + binning/fits (port of dNdS_analysis/utils/dynamics_utils.py).

Faithful port; the only change from the original is the ``config`` import
(``import dNdS_analysis.config as config`` -> ``from .. import config``) and the
fully-qualified ``config.data_path`` / ``config.blacklist_species`` /
``config.LiuGood2024_path`` aliases now live in ``dnds_dynamics.config``.
"""
